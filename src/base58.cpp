...
namespace {
    class CEMROAddressVisitor : public boost::static_visitor<bool> {
    private:
        CEMROAddress *addr;
    public:
        CEMROAddressVisitor(CEMROAddress *addrIn) : addr(addrIn) { }
...
bool CEMROAddress::Set(const CKeyID &id) {
    SetData(Params().Base58Prefix(CChainParams::PUBKEY_ADDRESS), &id, 20);
    return true;
}

bool CEMROAddress::Set(const CScriptID &id) {
    SetData(Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS), &id, 20);
    return true;
}

bool CEMROAddress::Set(const CTxDestination &dest) {
    return boost::apply_visitor(CEMROAddressVisitor(this), dest);
}

bool CEMROAddress::IsValid() const {
...
CTxDestination CEMROAddress::Get() const {
    if (!IsValid())
...
bool CEMROAddress::GetKeyID(CKeyID &keyID) const {
...
bool CEMROAddress::IsScript() const {
    return IsValid() && vchVersion == Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS);
}

void CEMROSecret::SetKey(const CKey& vchSecret) {
...
CKey CEMROSecret::GetKey() {
    CKey ret;
    ret.Set(&vchData[0], &vchData[32], vchData.size() > 32 && vchData[32] == 1);
    return ret;
}

bool CEMROSecret::IsValid() const {
    bool fExpectedFormat = vchData.size() == 32 || (vchData.size() == 33 && vchData[32] == 1);
    bool fCorrectVersion = vchVersion == Params().Base58Prefix(CChainParams::SECRET_KEY);
    return fExpectedFormat && fCorrectVersion;
}

bool CEMROSecret::SetString(const char* pszSecret) {
    return CBase58Data::SetString(pszSecret) && IsValid();
}

bool CEMROSecret::SetString(const std::string& strSecret) {
