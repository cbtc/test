...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeEMROAmount"))
        settings.setValue("nAnonymizeEMROAmount", 1000);
    nAnonymizeEMROAmount = settings.value("nAnonymizeEMROAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeEMROAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeEMROAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeEMROAmount:
            return QVariant(nAnonymizeEMROAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeEMROAmount:
            nAnonymizeEMROAmount = value.toInt();
            settings.setValue("nAnonymizeEMROAmount", nAnonymizeEMROAmount);
            emit AnonymizeEMROAmountChanged(nAnonymizeEMROAmount);
            break;
        default:
            break;
        }
