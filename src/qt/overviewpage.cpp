...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeEMROAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeEMROAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeEMROAmount = strAnonymizeEMROAmount.remove(strAnonymizeEMROAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeEMROAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeEMROAmount*COIN) nMaxToAnonymize = nAnonymizeEMROAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeEMROAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeEMROAmount));
        strAnonymizeEMROAmount = strAnonymizeEMROAmount.remove(strAnonymizeEMROAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeEMROAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
